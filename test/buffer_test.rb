...
class BufferTest < TestHelper
  ...
  test 'raise on overflow' do
    assert_raises(Buffer::Overflow) { @buf._read(1) }
  end

  test 'raise on invalid types' do
    assert_raises(Buffer::InvalidType) { @buf.read(:junk) }
    assert_raises(Buffer::InvalidType) { @buf.write(:junk, 1) }
  end
